...
namespace AlbumLieux.Services
{
	public abstract class BaseDataService
	{
		...
		private async Task SetAuthenticationToken(HttpRequestMessage msg, bool forceRefresh = false)
		{
			var token = _tokenService.Value.GetToken();
			if (forceRefresh || token is null || token.ExpiresAt < (DateTime.Now - TimeSpan.FromSeconds(30)))
			{
		...
		private async Task<HttpContent> SendRequest(HttpRequestMessage msg)
		{
			...
			{
				var newMsg = new HttpRequestMessage(msg.Method, msg.RequestUri);

				await SetAuthenticationToken(msg, true);
